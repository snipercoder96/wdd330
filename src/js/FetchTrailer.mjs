...
export async function displayTrailer(title, year = "", id) {
    ...
    try {
        ...
        const [tmdbResponse, creditsResponse] = await Promise.all([
            ...
            fetch(`https://api.themoviedb.org/3/movie/${id}/credits?api_key=${tmdbKey}`)
        ]);
        ...
        if (videoId) {
            ...
            <div class="grid-area2">
                ...
                <h1>${tmdbData.title} (${releaseYear})</h1>
                <div>
                ...
                <h2>Genres</h2>
                <div>
                    ${tmdbData.genres.map(g => `<p>${g.name}</p>`).join("")}
                </div>
                </div>
            ...
            addToFavBtn.addEventListener("click", () => {
                addToFavourites(tmdbData, year, tmdbData.genres);
            });
        } else {
            container.innerHTML = `
            <div>
